...
void solve(){
	int n;cin>>n;
	vector<vi>g(n+1);
	vector<set<int>>ed(n+1),ans(n);
	rep(i,0,n-1){
		int u,v;cin>>u>>v;u--,v--;
		g[u].pb(v);g[v].pb(u);
		ed[u].insert(v);
	}
	vi indp(n+1,0),outdp(n,inf);
	function<void(int,int)>dfs1=[&](int node,int p){
		indp[node]=0;
		for(auto ch:g[node]){
			if(ch==p)continue;
			if(ed[node].find(ch)==ed[node].end())indp[node]++;
			dfs1(ch,node);
			indp[node]+=indp[ch];
		}
	};
	...
	function<void(int,int)>dfs3=[&](int node,int p){
		...
		amin(res,outdp[node]);
		ans[outdp[node]].insert(node);
		 for(auto ch:g[node]){
		 	if(ch==p)continue;
		 	indp[node]-=indp[ch];
		 	indp[ch]+=indp[node];
		 	if(ed[ch].find(node)==ed[ch].end())indp[ch]++;
		 	else indp[ch]--;
		 	dfs3(ch,node);
		 	if(ed[ch].find(node)==ed[ch].end())indp[ch]--;
		 	else indp[ch]++;
