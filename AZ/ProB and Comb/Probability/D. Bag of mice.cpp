...
map<pair<int, int>, double> memo;
    function<double(int,int)> rec = [&](int W,int B) {
    	if(W<=0)return 0.0; 
    	if(B<=0)return 1.0;
    ...
    if (memo.find(args) != memo.end()) {
    	...
    	double ans=W*1.0/(W+B);//alice take white
    	double probB=B*1.0/(W+B);//alice can take black
    	B--;
    	probB*=B*1.0/(W+B);//continuesly king take black;
    	B--;
    	double nxtprob=0.0;//jump prob;
    	if(probB>1e-13){
    		nxtprob+=rec(W,B-1)*(double)(B)*1.0/(W+B);//jump black
    		nxtprob+=rec(W-1,B)*(double)(W)*1.0/(W+B);//jump white
    		ans+=nxtprob*probB;
    	}
    	return memo[args]=ans;
    };
